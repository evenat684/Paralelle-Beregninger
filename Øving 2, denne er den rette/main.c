...
void haloExchange(int halo_size, bmpImage *sub_image){
    int N_neighbours;
    int *my_neighbours = calloc(2, sizeof(int));

    //Figure out which ranks has 1 or 2 neighbours
    ...
    //set neighbours
    my_neighbours[0] = my_rank - 1;
    my_neighbours[1] = my_rank + 1;

    //Initialize variables for asyncrounous sends 
    //need one request and one status for each isend and irecv
    MPI_Request *reqs = calloc(sizeof(MPI_Request), N_neighbours * 2);
    MPI_Status *stas = calloc(sizeof(MPI_Status), N_neighbours * 2);
    int buffersize = sub_image->width*halo_size*sizeof(pixel);
    int *indexes = calloc(2, sizeof(int));

    //Set indexes for sends and receives
    indexes[0] = 0;
    indexes[1] = sub_image->height - halo_size;
    int i = 0;
    
    //start receives of south
    if(my_rank != 0){
      MPI_Irecv(sub_image->data[indexes[0]], buffersize,
                MPI_BYTE, my_neighbours[0], 0, MPI_COMM_WORLD,
                &reqs[i]);
    i+=1;
    }
    ...
    if(my_rank != comm_sz-1){
      MPI_Irecv(sub_image->data[indexes[1]], 
                buffersize, MPI_BYTE, my_neighbours[1], 0, MPI_COMM_WORLD,
                &reqs[i]);
    i+=1;
    }
    ...
    //north borders send
    if(my_rank != comm_sz-1){
      MPI_Isend(sub_image->data[indexes[1] - halo_size],
              buffersize,
              MPI_BYTE, my_neighbours[1], 
              0, MPI_COMM_WORLD,
              &reqs[i]);
    i+=1;
    }
    //South border sends
    if(my_rank != 0){
      MPI_Isend(sub_image->data[indexes[0] + halo_size],
                buffersize, MPI_BYTE, my_neighbours[0], 0, MPI_COMM_WORLD,
                &reqs[i]);
    i+=1;
    }
    ...
    MPI_Waitall(2 * N_neighbours, reqs, stas);
