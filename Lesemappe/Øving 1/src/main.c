...
int main(int argc, char **argv) {
    ...
    if (my_rank < 2){
        printf("My rank: %d, comm size: %d", my_rank, comm_sz);
        int N = 10000;
        int *data_send1 = calloc(N,sizeof(int));
        int *data_rcv1 = calloc(N,sizeof(int));
        int *data_send2 = calloc(N,sizeof(int));
        int *data_rcv2 = calloc(N,sizeof(int));
        ...
        MPI_Recv(data_rcv1, N, MPI_INT, n1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);


        MPI_Send(data_send2, N, MPI_INT, n2, 0, MPI_COMM_WORLD);

        MPI_Recv(data_rcv2, N, MPI_INT, n2, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        printf("Kom vi hit??? \n");
        printf("Første element %d \n", *data_rcv1);
        printf("Andre element %d \n", *data_rcv2);
    }
...
static void master_crack(ExtendedCrackResult *result, Options *options, Dictionary *dict, ShadowEntry *entry) {
    ...
    //receive buffers
    CrackJob *slave_job = calloc(1, sizeof(CrackJob));
    CrackResult *slave_result = calloc(1, sizeof(CrackResult));

    //Scatter buffers
    CrackJob *jobs = calloc(comm_sz, sizeof(CrackJob));
    CrackResult *results = calloc(comm_sz, sizeof(CrackJob));
    ...
    while(result->status == STATUS_PENDING) {
        ...
        //Broadcast the current result, ranks will need to terminate if result is found
        MPI_Barrier(MPI_COMM_WORLD);
        MPI_Bcast(result, sizeof(result), MPI_BYTE, 0, MPI_COMM_WORLD);        // TODO for all ranks
        ...
        MPI_Scatter(jobs, sizeof(CrackJob), MPI_BYTE, 
                    slave_job, sizeof(CrackJob), MPI_BYTE, 0, MPI_COMM_WORLD);
