...
bool read_dictionary(Dictionary *dict, Options *options, char const *path) {
    ...
    dict->elements = (char (*)[MAX_DICT_ELEMENT_LENGTH + 1]) malloc(dict->max_length * (MAX_DICT_ELEMENT_LENGTH + 1));
    ...
    while (getline(&line, &line_length, file) != -1) {
        ...
        if (dict->length == dict->max_length) {
            char (*old_elements)[MAX_DICT_ELEMENT_LENGTH + 1] = dict->elements;
...
bool get_next_shadow_entry(ShadowEntry *entry, FILE *file) {
    ...
    for (size_t i = 0; i < 2; i++) {
        ...
        switch (i) {
        case 0:
            strncpy(entry->user, token, MAX_USER_LENGTH);
            entry->user[MAX_USER_LENGTH] = 0;
            break;
        case 1:
            strncpy(entry->passfield, token, MAX_PASSFIELD_LENGTH);
            entry->passfield[MAX_PASSFIELD_LENGTH] = 0;
            break;
        default:
            break;
        }
...
static void parse_shadow_passfield(ShadowEntry *entry) {
    ...
    size_t passfield_length = strnlen(entry->passfield, MAX_PASSFIELD_LENGTH);
    for (size_t i = 0; i < passfield_length; i++) {
